...
def retrive_data(name):
    ...
    retrive_df.columns = ['name_role', 'facial_features']  # Fix the column name here
    retrive_df[['Name', 'Role']] = retrive_df['name_role'].apply(lambda x: x.split('@')).apply(pd.Series)
    return retrive_df[['Name', 'Role', 'facial_features']]  # Fix the column name here
names_to_delete = ['ronaldo', 'Abhigna Ragala']

for name in names_to_delete:
    r.hdel('academy:register', f'{name}@Student_21WU0102047')
    # Alternatively, if you want to delete the entire key
    r.delete(f'{name}@Student_21WU0102047')
for name in names_to_delete:
    r.hdel('academy:register', f'{name}@Student_21WU0102051')
    # Alternatively, if you want to delete the entire key
    r.delete(f'{name}@Student_21WU0102051')
...
class RealTimepred:
    ...
    def saveLogs_redis(self):
        ...
        try:
            ...
            print("Error in saveLogs_redis:", e)
